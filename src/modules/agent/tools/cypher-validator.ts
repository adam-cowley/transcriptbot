...
export class CypherValidator {
    ...
    async reload(): Promise<void> {
        ...
        for (const [from, details] of Object.entries(rows)) {
            if (details.type === 'node') {
                ...
                for (const [type, relDetails] of Object.entries(details.relationships)) {
                    if (relDetails.direction == 'out') {
                        for (const to of relDetails.labels) {
                            relationships.push(new Relationship(from, type, to, relDetails.properties))
                        }
                    }
                }
            }
    ...
    async getSchema(): Promise<string> {
        ...
        const properties = (node: Node | Relationship): string => '{' + Object.entries(node.properties)
            .map(([k, v]) => `${k}: ${v.type}`)
            .join(', ') + '}'

        const nodes = `Nodes:\n- ${this.nodes.map(node => `(:${node.label} ${properties(node)})`).join('\n- ')}`
        const relationships = `Relationships:\n- ${this.relationships.map(relationship => `(:${relationship.from})-[:${relationship.relationship} ${properties(relationship)}]->(:${relationship.to})`).join('\n- ')}`
    ...
    static async load(graph: Neo4jGraph): Promise<CypherValidator> {
        const validator = new CypherValidator(graph)
     ...
     */
    private anyRelationshipExists(from: string[], rel: string[], to: string[]): RelationshipExistsDecision {
        for (const f of from) {
            for (const t of to) {
                for (const r of rel) {
    ...
    private relationshipExists(from: string, rel: string, to: string): boolean {
        if (from === '') {
            return this.relationships.some(schema => schema.relationship == rel && schema.to == to)
        }
        else if (to === '') {
            return this.relationships.some(schema => schema.relationship == rel && schema.from == from)
        }
        else {
            return this.relationships.some(schema => schema.relationship == rel && schema.from == from && schema.to == to)
        }
    }
    ...
    validate(query: string): { query: string, errors: string[] } {
        ...
        for (const node of query.matchAll(nodePattern)) {
            ...
            for (const label of labels) {
                if (!label.includes('.') && label.trim() !== '' && !this.verifyNodeLabel(label)) {
